...
def _upload_and_analyze(client, pcap_bytes: bytes, target_ip: str = "192.168.1.2") -> str:
...
class TestCompareEndpointSuccess:
    def test_compare_two_uploads_returns_200(self, pcap_bytes):
        client = _make_app()
        uid_a = _upload_and_analyze(client, pcap_bytes)
        uid_b = _upload_and_analyze(client, pcap_bytes)
    ...
    def test_compare_same_capture_no_new_ips(self, pcap_bytes):
        ...
        uid_a = _upload_and_analyze(client, pcap_bytes)
        uid_b = _upload_and_analyze(client, pcap_bytes)
        body = client.post("/api/compare", json={"base_upload_id": uid_a, "current_upload_id": uid_b}).json()
        assert body["new_ips"] == []
